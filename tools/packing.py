...
class BottomLeftFill(object):
    def __init__(self,width,area,original_polygons,**kw):
    ...
    def showAll(self):
        ...
        for i in range(0,len(self.polygons)): 
            ...
            best_list.append(self.polygons[i])
            plots=np.append(plots,Polygon(self.polygons[i]))
        # print(best_list)
        # 保存到json
        import json
        with open('best_list.json', 'w') as f:
            json.dump(best_list,f)
        gdf = gpd.GeoDataFrame(geometry=plots)
        fig, ax = plt.subplots()
        # 绘制并填充颜色
        gdf.plot(color='blue', edgecolor='black', alpha=0.5)
        length=self.contain_length
        # 限定 x 和 y 轴范围
        ax.set_xlim(0, self.width)  # 设置 x 轴范围
        ax.set_ylim(0, length)  # 设置 y 轴范围
        rate=self.area/(self.width*length) # 填充率
        print('area:',self.area,'minL:',length,'rate:',rate)
        # 显示图形
        plt.show()
        ...
        # PltFunc.showPlt(width=self.width,height=length,minus=100)    
...
class PolyListProcessor(object):
    ...
    @staticmethod
    def randomRotate(poly_list,all_rotation,target_id):
        new_poly_list=copy.deepcopy(poly_list)

        poly_shape = Polygon(new_poly_list[target_id].poly)   
        ...
        new_poly_list[target_id].poly = [[x, y] for x, y in coords] 
        return new_poly_list

    @staticmethod
    def cross(poly_list1,poly_list2,target_id,all_rotation):
        poly_list=copy.deepcopy(poly_list2)

        poly_shape = Polygon(poly_list1[target_id].poly)   
        rotation_angle = random.choice(all_rotation)
        rotated_poly = affinity.rotate(poly_shape, rotation_angle)
        coords = list(rotated_poly.exterior.coords)[:-1]
        poly_list[target_id].poly = [[x, y] for x, y in coords] 
        return poly_list

    @staticmethod
    def mutate(best_list,poly_list1,poly_list2,poly_list3,poly_list4,F):

        # poly1=np.array(best_list[0].poly)+F*(np.array(poly_list1[0].poly[0])-np.array(poly_list2[0].poly[0])+np.array(poly_list3[0].poly[0])-np.array(poly_list4[0].poly[0]))
        # print(poly1)
        new_poly_list = copy.deepcopy(best_list)
        for i in range(int(len(best_list))):
            poly1=np.array(best_list[i].poly)+F*(np.array(poly_list1[i].poly[0])-np.array(poly_list2[i].poly[0])+np.array(poly_list3[i].poly[0])-np.array(poly_list4[i].poly[0]))
            new_poly_list[i].poly=poly1.tolist()

        swap_with = int(random.random() * len(new_poly_list))
        target_id = int(random.random() * len(new_poly_list))
        item1 = new_poly_list[target_id].poly
        item2 = new_poly_list[swap_with].poly
            
        new_poly_list[target_id].poly = item2
        new_poly_list[swap_with].poly = item1
        return new_poly_list

    @staticmethod
    def showPolyList(width,area,poly_list):
        blf = BottomLeftFill(width,area,PolyListProcessor.getPolysVertices(poly_list))
        blf.showAll()

    def getLength(width,area,poly_list):
        blf = BottomLeftFill(width,area,PolyListProcessor.getPolysVertices(poly_list))
        return blf.contain_length

    def getLength1(width,area,poly_list):
        blf = BottomLeftFill(width,area,PolyListProcessor.getPolysVertices(poly_list))
        blf.showAll()
        return blf.contain_length
