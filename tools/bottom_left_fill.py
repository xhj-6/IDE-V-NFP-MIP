...
class BottomLeftFill(object):
    def __init__(self,width,original_polygons,**kw):
        ...
        if 'is_first_call' in kw:
            self.is_first_call = kw['is_first_call']
    ...
    def showpoly1(self,i):
        # for i in range(0,2):
        for j in range(0,i):
            PltFunc.addPolygon(self.polygons[j])
        length=self.contain_length
        # PltFunc.addLine([[self.width,0],[self.width,self.contain_height]],color="blue")
        PltFunc.showPlt(length=self.width,width=length,minus=100)  
...
if __name__=='__main__':
    ...
    bfl=BottomLeftFill(38,polys,vertical=False,NFPAssistant=nfp_ass)
